...
function GroceryContent({ user }) {
  ...
  return (
    <AppFrame user={user} title="Grocery list" subtitle="Ingredients are combined from every planned lunch and dinner." action={<button className="primary-btn" onClick={copy}>Copy list</button>}>
      ...
      <div className="grocery-summary panel-soft">
        ...
        <div><span>Combined items</span><strong>{list.length}</strong></div>
        <div><span>Estimated week total</span><strong>{price(weekTotal)}</strong></div>
      </div>
          ...
          <section className="card category" key={category}>
            ...
            {items.map((item) => (
              <div className="grocery-item" key={`${item.name}-${item.unit}`}>
                <input type="checkbox" />
                <div><strong>{item.name}</strong><small>Used in: {item.meals.join(', ')}</small></div>
