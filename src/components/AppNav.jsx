    ...
    <>
      ...
      <nav className="mobile-bottom-nav" aria-label="Main navigation">
        {links.map(([label, href, icon]) => {
          ...
          return (
            <Link key={href} className={`mobile-tab ${active ? 'active' : ''}`} href={href}>
              <svg viewBox="0 0 24 24" aria-hidden="true">
                <Icon type={icon} />
              </svg>
              <span>{label}</span>
        ...
        {user ? (
          <Link className={`mobile-tab ${pathname?.startsWith('/account') ? 'active' : ''}`} href="/account">
            <svg viewBox="0 0 24 24" aria-hidden="true"><Icon type="account" /></svg>
            <span>Account</span>
          </Link>
        ) : (
          <Link className="mobile-tab login-tab" href="/login">
            <svg viewBox="0 0 24 24" aria-hidden="true"><Icon type="account" /></svg>
            <span>Login</span>
          </Link>
        )}
