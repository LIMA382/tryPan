  ...
  return (
    <form className="card meal-form" onSubmit={submit}>
      ...
      <div className="form-grid">
        <div className="field">
          <label htmlFor={`${formId}-title`}>Meal name</label>
          <input id={`${formId}-title`} name="title" required value={meal.title} onChange={(event) => update('title', event.target.value)} placeholder="Chicken rice bowl" />
        </div>

        <div className="field">
          <label htmlFor={`${formId}-meal-type`}>Meal type</label>
          <select id={`${formId}-meal-type`} name="mealType" value={meal.meal_type} onChange={(event) => update('meal_type', event.target.value)}>
            <option value="both">Any meal</option>
        ...
        <div className="field">
          <label htmlFor={`${formId}-prep-time`}>Prep time</label>
          <input id={`${formId}-prep-time`} name="prepTime" type="number" min="0" value={meal.prep_time} onChange={(event) => update('prep_time', event.target.value)} />
        </div>

        <div className="field">
          <label htmlFor={`${formId}-servings`}>Servings</label>
          <input id={`${formId}-servings`} name="servings" type="number" min="1" value={meal.servings} onChange={(event) => update('servings', event.target.value)} />
        </div>
        ...
        <div className="field full">
          ...
          <textarea id={`${formId}-description`} name="description" value={meal.description} onChange={(event) => update('description', event.target.value)} placeholder="A short note about this meal…" />
        </div>

        <div className="field full">
          <label htmlFor={`${formId}-instructions`}>Instructions optional</label>
          <textarea id={`${formId}-instructions`} name="instructions" value={meal.instructions} onChange={(event) => update('instructions', event.target.value)} placeholder="Write the cooking steps here, or leave empty if you already know the recipe." />
        </div>

        <div className="field full">
          <label htmlFor={`${formId}-video-url`}>Video link optional</label>
          <input id={`${formId}-video-url`} name="videoUrl" value={meal.video_url} onChange={(event) => update('video_url', event.target.value)} placeholder="YouTube, TikTok, Instagram, blog link…" />
        </div>

        <div className="field full">
          <label htmlFor={`${formId}-visibility`}>Visibility</label>
          <select id={`${formId}-visibility`} name="visibility" value={meal.is_public ? 'public' : 'private'} onChange={(event) => update('is_public', event.target.value === 'public')}>
      ...
      <div className="grid ingredients-grid">
        {meal.ingredients.map((ingredient, index) => (
          <div className="ingredient-row" key={index}>
            ...
            <div className="field">
              <label>Qty</label>
              <input aria-label={`Quantity for ${ingredient.name || 'ingredient'}`} type="number" min="0" step="0.01" value={ingredient.quantity} onChange={(event) => updateIngredient(index, 'quantity', event.target.value)} />
            </div>

            <div className="field">
              <label>Unit</label>
              <input aria-label={`Unit for ${ingredient.name || 'ingredient'}`} value={ingredient.unit} onChange={(event) => updateIngredient(index, 'unit', event.target.value)} placeholder="g" />
            </div>
