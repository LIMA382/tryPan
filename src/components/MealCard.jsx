...
export default function MealCard({ meal, actions, publicView = false, compact = false }) {
  return (
    <article className={`card meal-card ${meal.is_public ? 'public' : ''} ${compact ? 'compact' : ''}`}>
      ...
      {meal.video_url && (
        <a className="video-link" href={meal.video_url} target="_blank" rel="noreferrer">
          Watch recipe video
        </a>
      )}

      {meal.instructions && !compact && (
